...
配線・注意点は各ファイルの先頭コメント参照。無印ESP32向けと、手持ちのESP32-C3
SuperMini向け(`-c3`サフィックス、ピン配置はdocs/piezo.md §4準拠)の両方を用意した。
