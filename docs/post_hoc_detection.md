...
device2単独ピークの時刻そのものは相関的に背景以下だったが、S波到達直後のコーダ域
（上記1.の通り物理的に妥当な位置）に背景を上回る相関上昇が見つかり判定を上げた例がある）。
P窓・S窓という狭い窓のSNR/直線性だけでなく、到達後しばらく（コーダ域）まで`--corr-bin`で
見ることで、狭い窓では見えない根拠が見つかることがある。**「窓の中で光らなかった＝地震
ではない」と早合点しないこと。**
