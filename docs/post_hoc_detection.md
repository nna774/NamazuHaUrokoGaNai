...
## 0.5. （任意）投げる価値があるか目安で確認する

[docs/detection_range.md](detection_range.md)に、マグニチュード別「投げる価値ありレンジ」の表がある
（過去の事例から回帰した目安。`tools/detection_range.py`で再生成）。震源距離がこのレンジより
明確に近ければほぼ確実に捕れているので後回しでよく、明確に遠ければ恐らく埋没する
（それでも埋没側の実例を増やす目的なら価値はある）。レンジ内、または迷ったら手順1に進む。
これはあくまで事前の当たりをつけるためのショートカットで、**この目安だけで「捕れていない」と
結論しない**——必ず手順2のdetectlab解析まで確認すること。

候補を人間が気づいたものだけに頼らず機械的に洗い出したい時は`tools/scan_quakes.py`
（気象庁の公開地震一覧から「投げる価値あり」「恐らく埋没(埋没側の実例集め用)」を抽出、
`--eew`込みの`detectlab.py`呼び出しまで出す）を使う。詳細は[docs/detection_range.md](detection_range.md#使い方)。
...
**`--out`は必ず付ける。** 付けないと`plt.show()`でGUIウィンドウが開いてプロセスが
...
**片方の機体だけが孤立したSTA/LTAピークを作った時は、そのピーク時刻を鵜呑みにせず
`--corr-bin`で確認する。** 単発ピークが本物の揺れなら2機の直線性相関もその時刻付近で
上がるはずで、逆に相関が背景と同水準（またはそれ以下）ならそのピークは片方の機体固有の
ノイズと判断できる（[2026-08-26福島県沖M4.5/茨城県南部M3.4の事後解析](log/2026-08-26-fukushima-oki-m4.5-post-hoc-detection.md)で、
device2単独ピークの時刻そのものは相関的に背景以下だったが、S波到達直後のコーダ域に
背景を上回る相関上昇が見つかり判定を上げた例がある）。P窓・S窓という狭い窓のSNR/直線性
だけでなく、到達後しばらく（コーダ域）まで`--corr-bin`で見ることで、狭い窓では見えない
根拠が見つかることがある。
