...
- **RAMキューからLittleFSへの退避は「RAMキューが溢れた時」と「OTA直前」にしか起きない。** クラッシュや電源断の瞬間にRAMキュー(`kMaxRamBatches`: 現行esp32dev機=6本/3分、adxl355機=3本/45秒)に残っていた分は失われる。退避済みのファイルはLittleFSが電源断への耐性を設計目標にしているため、書き込み完了後(`close()`が返った後)なら安全。実測: 2026-08-07の家庭内ネット障害でdevice1が復旧に失敗し、約70分間1本もLittleFSへ退避できなかった（単なるWiFi未接続なら`uploaderTask`のループ構造上20秒おきに退避処理へ到達するはずなので、タスクごとフリーズしていたと推定）。詳細は[log/2026-08-08-device1-outage-and-deploy-drift.md](log/2026-08-08-device1-outage-and-deploy-drift.md)
...
- クラッシュ直前にRAMキューをLittleFSへ退避する経路（`flushToSpill()`はOTA前専用。パニックハンドラやtask watchdogのフックに仕込めるか）
- 物理ボタン長押しでの手動`flushToSpill()`（uploaderTaskが生きていて特定の待ちで固まっているだけの場合しか救えない、という限界込みで設計する）
- バックフィル起因のクラッシュ（上記）の原因特定・対策（送信間隔にバックオフを入れる等）
