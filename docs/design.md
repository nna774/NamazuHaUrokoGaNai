...
- **バックフィル起因のクラッシュ（device2で発見、上記）の対策。実装するのは(1)接続使い回しで合意（着手する）。定期再起動は不採用**——地震はいつ来るか分からず、メンテナンス目的の再起動と本震のタイミングが重なるリスクを許容できないため、意図的に選択肢から外した。代わりに検討している案（優先度順）:
  1. **実装したい方向で合意: `Uploader::postBatch()`が毎回ローカルで新規に`WiFiClientSecure`/`HTTPClient`を作りTLSハンドシェイクをやり直している(`batch-uplink/src/Uploader.cpp`)のを、バックログ吸い出し中は接続を使い回す(keep-alive)ように直す。** 重いのはハンドシェイク(鍵交換・証明書検証)であって確立済み接続上のデータ転送は軽いので、ワイヤ形式やingest側には一切触れずに「バックフィル中のハンドシェイク回数」を激減させられる。変更範囲が`batch-uplink`の`Uploader`内に閉じるのも利点
