...
- **新しい仮説（2026-08-08）: `uploaderTask`のtask watchdog(10秒・`trigger_panic=true`、`main.cpp`)が、`WiFiClientSecure`のTLSハンドシェイクの内部タイムアウト(120秒、`WiFiClientSecure.cpp`の`handshake_timeout = 120000`)より先に発火し、ハンドシェイク中に強制パニック再起動させている可能性がある。** ハンドシェイクの待ちループ(`ssl_client.cpp`)は`esp_task_wdt_reset()`を一度も呼ばない（呼べるのはUploader/main.cppのコードで、ライブラリの奥のこのループには手が届かない）ので、ネット瞬断でハンドシェイクが詰まると10秒でWDTが先に勝つ。パニック再起動は`flushToSpill()`を経由しないため、RAM上のバッチは毎回失われる。
  ...
  - **未検証。裏付けはreset_reasonテレメトリ配信後の次の再起動を待つ（2026-08-09時点、device2にOTA配信予定）。** [heapテレメトリ](log/2026-08-08-heap-telemetry.md)（実機配信済み）と合わせて見れば、この説と「バックフィル起因のヒープ枯渇」説(下記)を切り分けられる。
  ...
  - **実バックトレースで初めて確定(2026-08-29、device2で再発)。** 上記は全てタイミングの
    一致や意図的な全断実験からの推測だったが、device2のTASK_WDT再起動をESP-IDFの
    coredump-to-flash機構（既定で有効・パーティションも確保済みだったが今回初めて存在に
    気づいた）で読み出したところ、`uploaderTask`が`WiFiClientSecure::connect()`→
    `start_ssl_client()`のTLSハンドシェイク待ちループで20秒間WDTを養えず停止している
    実際のスタックを確認できた。**この機構自体は正しかったと確定したが、既存の緩和策
    (`setHandshakeTimeout(4000)`・WDT 20秒化)だけでは今回のケースを防げていない。**
    `ssl_client.cpp`を読むと、ハンドシェイクタイムアウトは`mbedtls_ssl_handshake()`の
    呼び出し**間**でしか働かず、個々の呼び出し内部のソケット`recv()`のタイムアウト
    (`SO_RCVTIMEO`)は接続タイムアウト(5000ms)から流用された別物——TCP接続確立・
    ハンドシェイクループ内の重いrecv・ハンドシェイク自体の判定が積み上がると、
    個々の値は妥当でも合計がWDTの20秒に迫りうる。
    coredumpの読み出し手順（fw_versionと同じコミットでelfを再現、`parttool.py`の
    簡易スタブ、ビルド日時差によるSHA256不一致の回避）を含め詳細は
    [log/2026-08-29-device2-task-wdt-coredump-tls-handshake.md](log/2026-08-29-device2-task-wdt-coredump-tls-handshake.md)
  - **修正方針を決定(2026-08-29、実装は未着手)。** 「各区間に明示的な締切を足し、
    `postBatch()`がタイムアウトでもパニックせず必ず`true`/`false`で戻るようにする」
    方向に決めた。区間ごとの上限を洗い出したところ、DNS解決を除いても
    TCP接続(5000ms)＋ハンドシェイク(判定4000ms＋直前の重いrecv一発分5000ms)＋
    レスポンスヘッダ読み取り(約5010ms、`available()`を10msポーリング)で合計約19秒に
    達しており、**さらにレスポンスボディ読み取り(`writeToStreamDataBlock`)には
    タイムアウト判定が一切無く事実上無制限**と判明した——`available()==0`の間
    `delay(1)`で回り続けるだけで、ここを塞がない限り「合計」という前提が成立しない。
    ここへの明示的タイムアウト追加を最優先とする。DNS解決(`WiFi.hostByName()`、
    `HTTPClient`側の設定が一切効かずlwIP既定値に丸投げ)の締切は今回は保留、後日
    別途対応する。他に検討した案（`postBatch()`中に進捗コールバックでWDTを養う→
    ハングが単一の`HTTPClient::POST()`呼び出し内部でvendored frameworkコードの
    奥にあるため届かず却下／WDTをさらに伸ばす→2026-08-09に一度実施済みで再発した
    場当たり対応のため却下）。副産物として、coredumpの経路を`Uploader::pump()`まで
    追い、パニックで実際に失われるのは**RAM上にある分だけ**（`ram_`は「pop→送信→
    成功でdelete・失敗でpush_front」の実装のため送信中は宙ぶらりん、加えて
    `gBatchQueue`・`samplingTask`の`cur`）で、**LittleFSへ退避済み(spill)は
    2xx確認後にしか`removeSpill()`しない設計のためパニックでも失われない**と
    確認できた。二重送信（送信成功後・応答受信前にクラッシュ/再送するケース）は
    どちらの経路でも起こりうるが、S3キーがバッチ内容由来で決定的なため冪等に
    上書きされ問題ない。**代替案「常時spill化」は却下ではなく保留**——摩耗は
    上記(2026-08-11試算、adxl355機104MB/日でも3〜6年)の通り問題にならないと
    分かっているが、健全時まで毎バッチにflash I/Oが乗り現行の速いRAM経由パスの
...
- **実装済み(2026-08-30、コード面。実機検証は未実施)。coredumpを自動でクラウドへ送る仕組み。** 2026-08-29の
  ...
  **設計方針(2026-08-29、さらに継続)。実装未着手のまま以下の形にほぼ収束。** クラウド側は
  新規Lambdaを立てず`lambda/ingest`に`POST /coredump`ルートを追加(既存`auth.verify()`を流用)、
  保存先は既存の非公開`data`バケットに`coredump/`prefixを新設し専用ライフサイクルで60日失効
  (`raw/`寄り、`events/`のような永久保持にはしない)、通知はwatchdogと同じSlackメンションを
  流用。デバイス側は単一スロット問題対策として**起動直後・WiFi接続前にcoredumpパーティションを
  LittleFS(`/coredump/`、`dropOldestWhenFull`なリングバッファ)へコピーしてから
  `esp_core_dump_image_erase()`で空ける**方式にした。アップロードは新しいtaskを作らず
  `setup()`内、`gUploader`生成・task起動より前の同期呼び出しにする——`tlsmempool::install()`
...
- **バックフィル起因のクラッシュ（device2で発見、上記）の対策。定期再起動は不採用**——地震はいつ来るか分からず、メンテナンス目的の再起動と本震のタイミングが重なるリスクを許容できないため、意図的に選択肢から外した。検討した案（優先度順）:
  ...
  4. **実装済み(2026-08-08)。heap free/maxblockをバッチ送信ヘッダで定期報告する。** `fw_version`・稼働時間と同じ`extraRequestHeaderNames/Values`の仕組みに`X-Namz-Heap-Free`/`X-Namz-Heap-Maxblock`を追加（4枠使い切り、**batch-uplink側の変更は不要**だった）。保存先は`namazu-devices`（上書き型の現在値台帳）ではなくCloudWatchカスタムメトリクス（`common/metrics.py`）を選んだ——上書きだと障害調査に要る推移が追えないため。maxblockは空き合計より断片化の兆候を直接示す（TLSハンドシェイクは大きな連続ブロックを要求するため）。CloudWatchの保持期間は解像度に応じて自動で粗くなる（1分解像度15日→5分63日→1時間455日）ので、直近の障害調査には十分な解像度が残る。これで次に実機で障害が起きた時、シリアルログ無しでも事後にCloudWatchのグラフで診断できるようになった。**実機配信済み(2026-08-08)**、device1/device2ともCloudWatchへの到達を確認した。
     ...
     **`X-Namz-Heap-Minfree`(ESP.getMinFreeHeap())を追加した(2026-08-30)。** free_heapは瞬間値なので一時的にしか出ないスローリークの兆候を見逃しうるが、起動後の生涯最小値ならリークがあれば必ず反映される（[docs/log/2026-08-30-esp32-hidden-features-survey.md](log/2026-08-30-esp32-hidden-features-survey.md)の棚卸しから）。`record_heap`/`latest_heap`とも第3引数はOptionalにし、旧ファーム(ヘッダ未送信)・データがまだ無い期間はキー自体を省く形で後方互換を保った
