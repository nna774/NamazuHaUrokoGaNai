...
- [ ] **【将来検討・優先度低】C=200nFがクランプ回避に必要な最小限より
      過剰に絞っている可能性がある**。今のCはカットオフ周波数から逆算した
      値で、「クランプを避けるのに必要な最小限の減衰量」から選んだ値では
      ない。ただし最適値を今すぐ計算で出すのは無理がある——切り分け実験時
      (23:30)にコンデンサを抜いた状態でもmin=0のクランプが8%程度発生して
      おり、クランプされている以上「本当の(クランプ無しの)ピーク振幅」が
      記録から読み取れないため。値を段階的に小さくして試す(ブレッドボード
      なので容易)か、オシロをNode A(Rs直前、クランプより手前)に当てて
      強めの風の瞬間の実振幅を直接見る必要がある。優先度は上記の地震動との
      余裕確認の結果次第——余裕が十分あるなら動いているものをいじる動機は
      薄く、余裕が無いなら真っ先に取るべき手になる
