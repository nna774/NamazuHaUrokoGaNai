...
東西・南北方向に LDT0-028K を 1 個ずつ設置する。`MCP6004` を 1 個使い、内部の
A/B を各軸の電荷増幅器、C を共有の `1.65V` 基準バッファへ割り当てる。未使用の D は
`1.65V` 基準を入力するボルテージフォロワとして終端する。東西軸は `GPIO0`、南北軸は
`GPIO1` へ接続する。帰還抵抗・帰還コンデンサ・入力抵抗・ADC 前段は軸ごとに用意する。

![LDT0-028K 東西・南北 2 軸電荷増幅器の配線図](img/ldt0-charge-amplifier.svg)

現行案のようにクランプ電流を ESP32 の 3.3 V レールへ直接流さない。入力保護、
非通電時の許容、オペアンプの型番は未決定であり、回路確定前に検討する。ブレッドボード
電源装置の故障はクランプによる逆給電の可能性を残すが、ピエゾの蓄積エネルギーは最大でも
約 `0.38µJ` と小さく、誤配線・逆接・USB／オシロ経由の別電源経路を含めて切り分ける。
詳細は[設計変更ログ](log/2026-09-08-ldt0-charge-amplifier-direction.md)を参照。
