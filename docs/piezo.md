...
- [ ] **アンチエイリアシングローパスを実装する**（§9「既存Z軸（圧電ブザー素子）
      側の設計」）。Node B-GND間にC=220nFを追加してfc≈15.39Hzの1次RCローパスに
      する。実装後、Serial出力(min/max/pp)でベースラインと反応の変化を確認する
...
- **共振(約50Hz)は`kSampleRateHz=100Hz`のナイキスト周波数(50Hz)とちょうど
  ...
  **風防(励起源を減らす)+ADC手前のアナログRCローパス(fsに依存せず除去可能に
  ...
  ADC手前のアナログRCローパスはLDT0-028K・Z軸(このブザー素子)とも値を決定
  済み（[§9](#9-アンチエイリアス対策-adc手前のrcローパスldt0-028kz軸とも値決定未実装)）。
  風除け筐体・実装(はんだ付け)・実機確認はまだ未着手。
...
クランプダイオードと同じノード）へコンデンサ1個をGNDへ足すだけの1次RCローパス。

```
fc = 1 / (2π × 47kΩ × 220nF) ≈ 15.39Hz
```

**C=220nF(E6系列)を採用する。** 1次フィルタの減衰は緩やか(-20dB/decade)なので、
50Hzでの減衰量は概算約10dB(振幅で約1/3程度)に留まる——共振を消し去るほど
急峻ではないが、対策ゼロの現状からは改善になる。この点も§9冒頭のLDT0-028K側と
同じ限界を持つ。実装・実機確認はまだ。
