...
ESP32に接続する前にオシロでピエゾ単体の出力を確認したところ、指で強く弾いただけで
**Vpp 17.64V**（ESP32のADC定格3.3Vを大きく超える）が出ており、その衝撃で素子自体も
破損した（在庫は複数あり実害無し。ESP32側は未接続だったため無事）。ピエゾ素子は
開放電圧が素子サイズに対して不釣り合いに高く出る（圧電ライターと同じ原理）ため、
「軽く弾く程度なら壊れる可能性は低い」という想定は誤りだった。軽く叩く程度でも
約1.5Vは出る。**保護回路は任意ではなく必須。**
...
- [ ] ピエゾ本体の静電容量を実測し、Rbの前提(仮定20nF)を検算する
      （§9の容量分圧仮説の検証にも要る値。手元にLCR系の直接測定機器が
      無いため間接測定の手法から検討すること）
...
- [x] **アンチエイリアシングローパスを実装する**（§9「既存Z軸（圧電ブザー素子）
      側の設計」）。Node B-GND間へ100nF×2並列(C=200nF、fc≈16.93Hz)を実装済み
      （220nFの手持ちが無かったための代替）
- [x] 実装したローパスの動作確認 → 本番のdashboardデータ(API `/recent`)で
      切り分け実験を実施し確認。詳細は§9「既存Z軸（圧電ブザー素子）側の設計」
      末尾参照
- [ ] **ローパス実装後、本物の地震動でも静穏化した新しいベースライン(pp~10台)
      に対して十分な余裕を持って浮き上がるかを確認する**。風ノイズとの
      相対比較(上記)はできたが、地震動そのものとの相対比較はまだ無い。
      回路+機械共振+ADCサンプリングを通したシミュレーションで一次近似の
      目安は得た（「初動レベルの揺れなら静穏帯からは浮き上がりそう」、
      [docs/log/2026-08-15-piezo-oversampling-boxcar-consideration.md](log/2026-08-15-piezo-oversampling-boxcar-consideration.md)）
      が、未校正センサでの目安計算に過ぎず実データでの確認ではない。
      [other-sensors.md §4](other-sensors.md#4-統合方針案-geophoneのフェーズ構造を流用)の
      phase2（検知限界の地震が来た時、既存イベントのタイミングとピエゾの
      生波形を後付けで突き合わせる）を待って確認すること
- [ ] **【将来検討・未実装】本線と同じ1kHzオーバーサンプリング+ボックスカー10平均
      をピエゾ側にも追加する**。シミュレーションでは、共振がナイキスト直下で
      叩き起こされた瞬間に生じる「サンプル毎にランダムに0⇄1000へ飛ぶ」エイリアシング
      の暴れを大きく抑える計算になった(P波初動直後のピークが約1/3に、詳細は
      [docs/log/2026-08-15-piezo-oversampling-boxcar-consideration.md](log/2026-08-15-piezo-oversampling-boxcar-consideration.md))。
      ただしESP32-C3はシングルコアでWiFi/TLSタスクと時間を取り合っており、
      1kHzの`analogRead()`を安定して回せるかは実機で確認が必要
- [ ] **【経過観察中】C=200nFがクランプ回避に必要な最小限より過剰に絞って
      いる可能性を実地で検証中**。C=100nF(半分)に変更し経過観察中（2026-08-15未明〜、
      §9「既存Z軸（圧電ブザー素子）側の設計」末尾「【現状・実験中】」参照）。
      当夜の風では問題無く見えたが、より強い風の日に再びクランプが出るかは
      まだ確認できていない。継続して様子を見ること
...
- **風で本体（むき出しのカンチレバー構造）が直接励振されている**。生波形のFFTで
  風の息の周期(0.03〜0.1Hz)のエンベロープに約50Hz(§5実測の固有振動数と一致)の
  リンギングが乗っており、机を叩いた時と同じ「衝撃→減衰」シグネチャだった
  （[docs/log/2026-08-12-piezo-wind-sensitivity-observation.md](log/2026-08-12-piezo-wind-sensitivity-observation.md)）
- **共振(約50Hz)が`kSampleRateHz=100Hz`のナイキスト周波数と重なり、エイリアシングで
  1-10Hz帯（震度算出の対象帯域）に折り返りうる**。次期センサLDT0-028Kのmass選定にも
  影響し(mass=1の共振90Hzは10Hzに直撃)、対策は**風防+ADC手前のアナログRCローパスを
  主軸**に決めた（オーバーサンプリング+間引きは狭帯域ノイズに効果が薄く不採用。詳細:
  [docs/log/2026-08-12-piezo-resonance-noise-diagnosis-and-mitigation-plan.md](log/2026-08-12-piezo-resonance-noise-diagnosis-and-mitigation-plan.md)）。
  Z軸側のRCローパスは実装・動作確認済み（[§9](#9-アンチエイリアス対策-adc手前のrcローパスldt0-028kz軸とも値決定未実装)）、
  LDT0-028K側は値の決定のみ、風除け筐体は未着手
...
## 9. LDT0-028Kの入力回路と既存Z軸のアンチエイリアス対策

### LDT0-028K側の現在の方針: 電荷増幅器

LDT0-028K の `430〜500MΩ` 受動入力案は撤回した。このインピーダンスでは基板表面の
漏れ、湿気、配線容量、商用電源由来の電界、ESP32-C3 の SAR ADC 入力の充放電の影響が
大きく、抵抗と配線自体がアンテナになるためである。`1.65V` を基準にした単電源の
電荷増幅器で受け、第一候補を `Cf=22nF`・`Rf=10MΩ` の並列帰還とする。
LDT0-028K の `480pF` と、記憶ベースの見積もり `80Vpp` (`±40Vpeak`、
開放電圧の実測値`63Vpp`に安全マージンを見て採用)から、最大出力変化は約
`±0.87V`、低域カットオフは約 `0.72Hz` となる。

東西・南北方向に LDT0-028K を 1 個ずつ設置する。オペアンプは `MCP6004-I/P`
(14-DIP)に確定し、内部の A/B を各軸の電荷増幅器、C を共有の `1.65V` 基準
バッファへ割り当てる。未使用の D は `1.65V` 基準を入力するボルテージ
フォロワとして終端する。東西軸は `GPIO0`、南北軸は `GPIO1` へ接続する。
入力直列抵抗は `220kΩ`(旧保護回路のRsを流用)に確定した。帰還抵抗・帰還
コンデンサ・ADC 前段は軸ごとに用意する。

![LDT0-028K 東西・南北 2 軸電荷増幅器の配線図](img/ldt0-charge-amplifier.svg)

**外付けクランプダイオードは不要と判断した。** MCP6004のデータシート
(入力ピン電流の絶対最大定格`±2mA`)によれば、内蔵ESDダイオード + 入力
直列抵抗による過電圧保護がメーカー推奨構成であり、`220kΩ`であれば
LDT0-028Kの想定最悪電圧(データシート`>70V`)でも電流は`約303µA`に収まり
定格に対して約7倍の余裕がある。ただし**3.3Vレールへの逆給電経路(現行案の
1N60と同じ問題)は、外付けダイオードから内蔵ESDダイオードへ場所を移した
だけで残っている**——電流が小さいことは以前の電源装置故障の機序(熱では
なく寄生経路のトリガーと推定)に対する安全の保証にはならないため、
ピエゾを弾いた瞬間と電源OFF時の3.3Vレールをオシロで観測するまでは
「解決済み」とは扱わない。詳細は[設計変更ログ](log/2026-09-08-ldt0-charge-amplifier-direction.md)・
[BOM確定ログ](log/2026-09-08-ldt0-charge-amp-bom-and-input-protection.md)を参照。

### LDT0-028K側の旧RCローパス案（撤回済み）

§7末尾で見つかった「共振(約50Hz)がfs=100Hzのナイキスト周波数とちょうど
重なる」問題への対策のうち、軸2b（アナログRCローパス）の定数を、LDT0-028K
がまだ届いていない(共振周波数が未実測)段階で先に決めた。経緯・計算は
[docs/log/2026-08-13-ldt0-028k-lowpass-filter-design.md](log/2026-08-13-ldt0-028k-lowpass-filter-design.md)参照。

**【予想・未検証、要再検討】** 以下のカットオフ計算はNode A(センサ側)を
理想電圧源とみなす近似に基づいている。LDT0-028Kの静電容量(C1=480pF、
データシート既知)に対しC=27nFはC1の56倍もあり、実際には周波数選択フィルタ
ではなく容量分圧器(≈1/57、-35dB相当、1-10Hz帯も共振も一律に削る)として
働いている可能性が高い。Z軸側で見つかった同種の問題・対応案の検討は
[docs/log/2026-08-14-piezo-lowpass-capacitive-divider-hypothesis.md](log/2026-08-14-piezo-lowpass-capacitive-divider-hypothesis.md)参照。
**実装前に、この文書の対応案を踏まえて設計を見直すこと。**

- **新しい抵抗は追加しない**: LDT0-028K用保護回路のNodeB(Rs=220kΩの直後、
  クランプダイオードと並列)へコンデンサ1個をGNDへ足すだけで、既存のRsが
  そのままローパスの抵抗を兼ねる。
- **カットオフは共振周波数を知らなくても決められる**: 1-10Hz帯を削らない
  下限と、ナイキスト(50Hz)より十分低い上限の2つの既知の制約から挟んで
  **20〜30Hz**に絞った。Rs=220kΩ固定で**C=27nF(候補、fc≈26.8Hz)**を
  第一候補とする。
- **1次フィルタ(6dB/oct)では急峻には落ちない**。mass=1(90Hz→fs=100Hzで
  畳み込むと1-10Hz帯上端の10Hzに直撃)は-10.8dB程度しか削れず、単独では
  信号帯への混入を防ぎきれない。対策の主軸は引き続き§7の軸1(風防)で、
  このローパスは補助という位置づけ。
- 実装・実機確認はまだ。LDT0-028K到着後、実測した共振周波数と減衰表
  （ログ参照）を照らしてCの値(E24系列の隣、22nF/33nF)を振るかどうか判断する。
  **ただし上記の容量分圧仮説が正しければ、この節の設計自体を先に見直す
  必要がある**（実装前に判断すること）。

### 既存Z軸（圧電ブザー素子）側の設計（実装済み・値は経過観察中に変更あり）

**現在の値はC=100nF。** Z軸は§4の保護回路のNode B（Rs=47kΩの直後）へ
コンデンサをGNDへ足すだけの1次RCローパスで、共振周波数(§5実測49.947Hz)を
基に「1-10Hz帯を削らない下限」「ナイキストより十分低い上限」から20〜30Hz帯へ
絞る設計にした（設計の詳細は[docs/log/2026-08-14-piezo-buzzer-lowpass-design.md](log/2026-08-14-piezo-buzzer-lowpass-design.md)）。

最初はC=200nF(fc≈16.93Hz)で実装し、本番dashboardデータでコンデンサの抜き差しを
比較する切り分け実験を行った結果、**静穏化の主要因はカンチレバーの付け直しでは
なくローパスコンデンサそのもの**と実データで確認できた——理論上の1次RC減衰
(-9.4dB)よりずっと大きい改善が出たのは、フィルタ前はADCが頻繁にクランプ(0付近)
へ張り付く飽和状態だったため、それが解消したことによる非線形な改善と考えられる
（詳細: [docs/log/2026-08-15-piezo-lowpass-value-tuning-experiment.md](log/2026-08-15-piezo-lowpass-value-tuning-experiment.md)）。
その後、200nFは過剰減衰ではという疑問から**C=100nFへ半減して様子見中**——
100nFでも当時の風では問題無く見えたため据え置いている。今後より強い風の日に
再びクランプが出るかどうかで200nFへ戻すか判断する。

**【予想・未検証】** 当初の`fc`計算はNode A(ピエゾ側)を理想電圧源とみなす近似に
基づいているが、実際はピエゾ自身の静電容量(C1、未実測)に対しC2(100〜200nF)が
10倍以上大きく、周波数に関係なく効く「容量分圧」が主効果で、狙った周波数選択性は
ほとんど効いていない可能性がある——だとすると1-10Hz帯の感度も同程度落ちている
ことになる。**LDT0-028K側(§9冒頭)の設計にも同じ問題が(より深刻な形で)当てはまる**。
詳細・対応案は[docs/log/2026-08-14-piezo-lowpass-capacitive-divider-hypothesis.md](log/2026-08-14-piezo-lowpass-capacitive-divider-hypothesis.md)参照。
