...
**【訂正】この節で報告していた実測結果は無効。** 実際にはピエゾをGPIO4へ
接続し忘れていた状態での測定だった。フローティングピンのADC値は元々ノイズが
乗りやすく、さらに指を近づける/触れる動作自体が静電結合でフローティングピンの
値を動かす（いわゆる「ゴーストタッチ」）。「無入力時pp~220-240、弾くとpp~700」
という観測はピエゾの信号ではなくこの現象だった可能性が高い。詳細は
[docs/log/2026-08-11-piezo-gpio4-not-connected.md](log/2026-08-11-piezo-gpio4-not-connected.md)。
GPIO4への実接続後に再検証すること。
...
- [ ] 保護回路込みの配線（§4）でGPIO4に接続し、phase0
      （足音・指弾き等に反応するかSerialで確認）を行う → **未達成に戻す**。
      前回「達成」としたのはGPIO4未接続(フローティングピン)の誤測定と判明
      （[docs/log/2026-08-11-piezo-gpio4-not-connected.md](log/2026-08-11-piezo-gpio4-not-connected.md)）
