...
- **共振(約50Hz)は`kSampleRateHz=100Hz`のナイキスト周波数(50Hz)とちょうど
  ...
  **風防(励起源を減らす)+ADC手前のアナログRCローパス(fsに依存せず除去可能に
  ...
  ADC手前のアナログRCローパスは、**Z軸(このブザー素子)側は実装・実データでの
  動作確認とも完了**（C=200nF、fc≈16.93Hz。コンデンサの抜き差しによる切り分け
  実験で静穏化の主要因と確認、詳細は[§9](#9-アンチエイリアス対策-adc手前のrcローパスldt0-028kz軸とも値決定未実装)）。
  LDT0-028K側は値の決定のみで実装はまだ。
  風除け筐体は未着手のまま。
...
## 9. アンチエイリアス対策: ADC手前のRCローパス（LDT0-028K・Z軸とも値決定・未実装）
...
**現在実装中の値はC=100nF（この節末尾「【現状・実験中】」参照）。** 以下は
最初の実装(C=200nF)時点の設計記録。

LDT0-028Kは共振周波数が未実測(到着前)だったため「1-10Hz帯を削らない下限」と
「ナイキストより十分低い上限」の2つの制約で20〜30Hzに絞る手探りの設計だった。
Z軸（§4の保護回路、Rs=47kΩ）は**共振周波数を§5で既に実測済み(49.947Hz)**なので、
同じ考え方をより直接的に適用できる（詳細・経緯は
[docs/log/2026-08-14-piezo-buzzer-lowpass-design.md](log/2026-08-14-piezo-buzzer-lowpass-design.md)）。

LDT0-028Kと同じ手法で、**新しい抵抗は追加しない**——Node B（Rs=47kΩの直後、
クランプダイオードと同じノード）へコンデンサをGNDへ足すだけの1次RCローパス。

設計時点ではC=220nF(E6系列)を狙っていたが、**手持ちの詰め合わせが100nFまでしか
無かったため、100nFを2個並列でC=200nFとして実装した**（詳細・並列の妥当性は
ログ参照）。

```
fc = 1 / (2π × 47kΩ × 200nF) ≈ 16.93Hz
```

1次フィルタの減衰は緩やか(-20dB/decade)なので、50Hzでの減衰量は概算約9.4dB
(振幅で約1/3程度)に留まる——共振を消し去るほど急峻ではないが、対策ゼロの
現状からは改善になる。この点も§9冒頭のLDT0-028K側と同じ限界を持つ。
Node B-GND間はブレッドボード上での実装（はんだ付けではない）。

**動作確認: 本番dashboardデータでの切り分け実験により、劇的な改善を確認した。**

実装直後(23:19起動)から明らかに静穏化したが、同時にカンチレバーの脱落・
付け直しも起きており（詳細はログ参照）、どちらの寄与か切り分けが必要だった。
ブレッドボード上の追加コンデンサ(100nF×2)だけを一時的に抜き差しし、
カンチレバーの状態は固定したまま前後を比較した（API `/recent` エンドポイント、
値は生ADC値ベース）:

| 状態 | 平均pp/バケット・レンジ(概算) | クランプ(0付近への張り付き) |
|---|---|---|
| 実装前(参考、別々の2晩の同時間帯) | ≈929〜1059 / ≈2000〜2066 | 頻発 |
| コンデンサ有り(1回目、カンチレバー付け直し後) | ≈7.6 / ≈252 | 無し |
| **コンデンサ抜き(カンチレバーは同一のまま)** | **≈435 / ≈2053** | **復活(6000バケット中487)** |
| コンデンサ挿し戻し後(直近1分、生データ) | stdev≈10.1 / レンジ37.3 | 無し |

カンチレバーを固定したままコンデンサだけを抜き差しして静穏⇄騒がしいが
再現した——**静穏化の主要因はカンチレバー付け直しではなくローパスコンデンサ
そのもの**と実データで確認できた。理論上の1次RC減衰(-9.4dB、振幅で約1/3)
より遥かに大きい改善(数十〜百倍オーダー)が出ている理由は、フィルタ前は
風などの励振で頻繁にADCがクランプ(0付近)へ張り付く飽和状態だったためと
推定される——50Hz成分がクランプ閾値を下回るところまで削れたことで飽和自体が
解消し、線形なdB計算では説明できない非線形な改善が乗ったと考えられる。

**【予想・未検証】** 上記とは別に、そもそも当初の`fc`計算自体がNode A
(ピエゾ側)を理想電圧源とみなす近似に基づいており、実際はピエゾ自身の
静電容量(C1、未実測)に対しC2(200nF)が10倍以上大きいため、周波数に関係なく
効く「容量分圧」(≈1/11、-20.8dB相当)が主効果で、狙った周波数選択性は
ほとんど効いていない可能性がある——だとすると1-10Hz帯の感度も同程度
落ちていることになる。検証はしていない仮説であり、**LDT0-028K側(§9冒頭)の
設計にも同じ問題が(より深刻な形で)当てはまる**。詳細・対応案の検討は
[docs/log/2026-08-14-piezo-lowpass-capacitive-divider-hypothesis.md](log/2026-08-14-piezo-lowpass-capacitive-divider-hypothesis.md)参照。

### 【現状・実験中】C=100nFへ半減し様子見中(2026-08-15未明)

上記の「200nFは過剰減衰では」という疑問（§6のTODO）を受け、当夜の風が
クランプ発生時と近い状態に見えたため実地で試した。**100nF×2並列のうち
片方を抜き、C=100nF(k≈5、理論値≈-15.6dB)に変更、カンチレバーは触らず**。
詳細は[docs/log/2026-08-15-piezo-lowpass-value-tuning-experiment.md](log/2026-08-15-piezo-lowpass-value-tuning-experiment.md)参照。

最初に取得したデータ(0:15スタート、10分窓)では最大pp≈1407・クランプ2件が
出て「200nFは過剰ではなかった」と一旦判断しかけたが、**開始時刻を1分ずらす
(0:16スタート)と最大pp≈46.1・クランプ0件に一変した**——0:15スタートの
異常値は、コンデンサを引き抜く操作そのものの機械的・静電的な衝撃が
1バケットに混入したアーティファクトだったと判断した。

**【教訓】実験操作の直後のデータは、操作自体のノイズが混入していないか
開始時刻をずらして再確認すること。** クランプ抜き差しの瞬間は特に、ピエゾに
物理的に触れる・静電気が乗る等でスパイクが入りやすい。

クリーンな0:16スタートのデータでは、C=100nF(平均pp≈17.2・最大pp≈46.1・
クランプ無し)はC=200nF(平均pp≈7.6・最大pp≈224.6・クランプ無し)と比べて
平均はやや大きいが最大ppはむしろ小さく、少なくとも当夜のこの時間帯の風では
問題無く見える。**C=100nFのまま様子見する方針とした**（実装済み、経過観察中）。
今後より強い風の日に再びクランプが出るかどうかで、200nFへ戻すかの判断材料が
増える見込み。
