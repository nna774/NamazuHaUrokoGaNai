...
- [x] **アンチエイリアシングローパスを実装する**（§9「既存Z軸（圧電ブザー素子）
      ...
      （220nFの手持ちが無かったための代替）
- [ ] 実装したローパスの動作確認。Serial出力(min/max/pp)でベースラインと
      反応の変化を確認する
...
- **共振(約50Hz)は`kSampleRateHz=100Hz`のナイキスト周波数(50Hz)とちょうど
  ...
  **風防(励起源を減らす)+ADC手前のアナログRCローパス(fsに依存せず除去可能に
  ...
  ADC手前のアナログRCローパスは、**Z軸(このブザー素子)側は実装済み**
  （C=200nF、fc≈16.93Hz、動作確認はまだ）。LDT0-028K側は値の決定のみで
  実装はまだ（[§9](#9-アンチエイリアス対策-adc手前のrcローパスldt0-028kz軸とも値決定未実装)）。
  風除け筐体は未着手のまま。
