...
デプロイ後、本番の4294967295番機で実際に確認した。

1. `tools/mute_device.py mute 4294967295`でmute（`/devices/4294967295`の
   `watchdog_muted`が`true`になることをAPI経由で確認）。
2. 実機の電源を入れてバッチ送信を開始 → ダッシュボードで自動unmuteされたのを
   確認。ingestの`watchdog_mute.clear_mute()`が想定通り動いた。
3. unmute後、watchdogが「デバイス復帰」をSlack通知（`devices.evaluate()`が
   `notified_at`有り・受信再開ありで"recovery"を返した経路）。
4. そのまま試験機を放置（黙らせた）ら、想定通り「デバイス欠測」の通知が1回
   来た。ここで再度`tools/mute_device.py mute 4294967295`を実行し、mute状態に
   戻した（`watchdog_muted`が`true`に戻ったことを確認済み）。

**残る検証**: このmuteのまま24時間放置し、`NAMZ_OFFLINE_RENOTIFY_S`(既定1日)後の
再送が来ないことを確認する。来なければ「試験のたび繋いでは再送スパムに悩まされる」
問題は解決とみなせる。
