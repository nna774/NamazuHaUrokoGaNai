...
### そもそも「コアダンプが吸い出せる」とはどういうことか

これまでdevice1/device2で再起動系の障害を調べる時は、**シリアルを直結してリアルタイムで
ログを流しっぱなしにし、たまたまその場でクラッシュが起きるのを待つ**しかなかった
（2026-08-08〜09のdevice1調査は全部この方式）。原因不明のまま何時間も張り付く必要があり、
再現しなければ何も得られない、効率の悪い方法だった。

**コアダンプ**は、プログラムが異常終了した瞬間の「現場の状態」——その時点で動いていた
全タスクのCPUレジスタ・コールスタック（どの関数からどの関数を呼んで、今どこにいたか）
一式のスナップショットを指す一般的な仕組み（PCで言うクラッシュダンプと同じ）。ESP32には
これをフラッシュメモリの専用領域に**自動で**保存する機能が標準で備わっており、しかも
この機体では**すでに有効になっていた**（後述）——つまりファームを1行も変えずに、
今この瞬間にも次のクラッシュの証拠を勝手に記録してくれる状態だった。これまで誰も
使っていなかっただけで、実は「シリアルに張り付く」を一切せずに済む方法が最初から
あったことになる。

これが分かった実利は大きい:

- **クラッシュの瞬間に立ち会う必要がなくなる。** 普段の設置場所に置きっぱなしのまま
  勝手に記録され、フラッシュに残る（次に別の原因でもう一度落ちるまで上書きされない）。
  後日思い立った時にUSBを挿すだけで、過去に起きたクラッシュの中身を読める。
- **今回まさにこれで助かった。** シリアルを挿していた約100分間は再現しなかったが、
  挿す**前**にすでに何度も起きていたクラッシュの記録がフラッシュに残っていたため、
  「新しく再現するまで待つ」ことなく、その場ですぐ過去の1件を読み出せた。
- 読み出しにはPython製の`esp-coredump`というツールと、当時動いていたファームと
  一致するビルド（シンボル解決用）が要る。今回は`fw_version`（gitのコミットハッシュ）を
  手がかりに再ビルドして用意した。詳しい手順は次の節。
