...
5. **SHA256不一致で一度弾かれた**（`coredump SHA256(7e4068acee9a0f50) != app
   SHA256(e79e4998e0f3d0c6)`）。ソースは同一コミットのため、`esp_coredump`の
   site-packagesは編集せず実行時モンキーパッチでこのチェックを警告に格下げして
   読み進めた。この時点では原因は「ビルド日時等の差だろう」という推測に留まっていた
6. **再ビルドしたバイナリが実際にビット一致するかは、後からS3で実証できた。**
   `tools/publish_ota.sh`は`ota/<env>/<version>.bin`をS3(`namazu-dashboard-*`バケット)へ
   アップロードするだけで消さないため、OTAで配った版なら実際に焼かれたバイナリが
   そのまま残っている。`976bd93`はOTAで配信済みだったので`aws s3 cp`で取得し、手元の
   再ビルド版と`cmp -l`でバイト比較したところ、**1,053,376バイト中32バイトしか
   違わなかった**（+末尾の32バイトは画像全体のチェックサムで、前者の差分がそのまま
   伝播しただけ）。しかもこの32バイトはオフセット177〜208、まさに
   `esp_coredump`が警告した`app SHA256`の値そのものが書き込まれている位置——
   `esp_app_desc_t.app_elf_sha256`フィールド自体だった。**つまりコード・rodata等の
   実体は完全にビット一致しており、「ビルド日時差だろう」という推測ではなく実測で
   裏付けが取れた。** バックトレースのシンボル解決は確実に正しい。
   （このフィールド自体がなぜビルドごとに変わるかは未調査——ビルドパス等、最終的な
   コード配置に影響しない何かがハッシュ計算にだけ紛れ込んでいると見られる）
