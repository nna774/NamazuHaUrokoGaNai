...
90日という数字自体を洗い直した。`git log`で確認したところ、**90という数字自体は
`plan.md`の初回コミット（2026-07-12 02:53、「リポジトリ骨格と設計ドキュメントを
追加」）に根拠なく置かれていて**、S3ライフサイクルの説明・バックテストの
正当化・コスト試算（「S3ストレージ~5GB（90日分）...≈月$1未満」）と、複数箇所に
同じ90が検証無しでそのまま使い回されていた。25分後の`terraform`導入コミット
（同日03:18、`raw_retention_days`のデフォルト）も`docs/design.md`の該当行も、
この`plan.md`の90をそれぞれ検証せず踏襲しただけで、データに基づく要件から
逆算した数字ではない。かつ**terraformを初めて導入したのがその2026-07-12で、今日
