...
- `jismo/rounding.py`に震度階級の境界値`SCALE_BOUNDARIES`と帯の中央値ラベル
  `SCALE_BAND_LABELS`を追加し、計測震度パネル（`detectlab.py`・`intensity_timeline.py`
  両方）にJMA震度階級（0,1,2,3,4,5弱,5強,6弱,6強,7）の目盛りを薄く重ねた。窓の先頭
  （`--eew`指定時は発生時刻より前=t<0）は必ず開いた背景ノイズ域なので、そこに
  ラベルを置く。現在のy範囲に収まる階級だけ出す（埋没例のように震度0の域を出ない
  記録では、境界線・ラベルとも一切出ない——それ自体が「震度0に収まっている」ことの
  視覚的な裏付けになる）。
