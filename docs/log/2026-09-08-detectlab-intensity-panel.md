...
- 計測震度パネル（`detectlab.py`・`intensity_timeline.py`両方）に、この記録の
  ピーク計測震度がJMA震度階級でいくつに当たるかを注記した。y軸の目盛りは既に数値で
  分かるので、階級への換算だけを添える形。ピーク点にはドットだけ打ち、テキストは
  各系列の左上に固定位置でまとめる——最初はピーク点そのものに文字を置いたが、
  ピークが上のパネルにはみ出したり複数機のピークがほぼ同時刻で重なったりして
  読みにくく、y範囲を広げて回避しようとしても記録によって必要な余白がまちまちで
  安定しなかった。データの形に依存しない固定位置の方が結局読みやすいと判断した。
