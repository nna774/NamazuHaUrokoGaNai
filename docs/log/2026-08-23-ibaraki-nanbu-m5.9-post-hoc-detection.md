...
## 訂正: 2分窓ズームで見えた本当の立ち上がり

```bash
python tools/detectlab.py --at "2026-08-23 02:00:50" \
  --eew "36.0,140.1,70,2026-08-23 02:00:50" --minutes 2 --device 1 2 \
  --out docs/log/img/2026-08-23-ibaraki-nanbu-m5.9-2min-zoom.png
```

2分窓で見ると、両機ともほぼ同時刻（絶対時刻02:01:03.7〜04頃）に**STA/LTA比≈24・
直線性≈0.95超という、この解析で見た中で最も明瞭で孤立したスパイク**があり、これが
1回目のP窓（02:01:11-18、誤った02:00:50仮定に基づく）より8〜10秒早い。
...
と推定できる。気象庁の丸め表記「02:00頃」とは矛盾しない。地震であるという結論は
揺るがない（むしろ孤立した鋭いスパイクという形はP窓が正しく置かれた場合より説得力が
高い）が、**正確な発生秒は現時点で公表されていないため、P窓/S窓を使った定量評価は
今回見送る**。

![2分窓ズーム(STA/LTA・直線性)](img/2026-08-23-ibaraki-nanbu-m5.9-2min-zoom.png)
