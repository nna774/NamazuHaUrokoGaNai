...
起こりうる。次に検討する選択肢（ユーザーとの相談中、2026-08-10時点で未定）:

1. TLSプールを実測ピーク(48304B)ぎりぎりまで削り、一般ヒープの余裕を増やす
   （TLS側のcalloc失敗リスクは上がるが、mbedTLSはC実装なので安全に失敗できる）
2. `kMaxRamBatches`を減らしRAMキューの最大占有量を下げる(既存の容量設計を変える)
3. `loadOldestSpillPath()`側の`new`失敗を先に直す
