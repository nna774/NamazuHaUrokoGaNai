...
# n=6の実例（detection_events.csv、2026-09-12時点）に基づく暫定値——成功例は
# 福島県沖M4.0(273km)・青森県東方沖M4.7(548km)。M4.0未満まで広げると震源距離~900kmに
# 対して物理的にほぼ検出不能な群発地震（九州M1.7〜3.0が大半）が月100件超流入するため、
# そこは拾わない。実例が増えるたびに見直す前提（毎週土曜のダイジェストでリマインドする）。
