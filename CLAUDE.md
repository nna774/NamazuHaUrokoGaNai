...
| 停電対策のUPS導入作戦（実機装着済み・稼働中） | [docs/ups.md](docs/ups.md) |
...
- **イベントのデータモデル**（DynamoDB `namazu-events`、[lambda/common/events.py](lambda/common/events.py)）:
  ...
  - **`api`の`/event`はCloudFrontで長期キャッシュしている**（確定済みは1年相当、速報のみは
    ...
    （`from`/`to`もwhitelistされているが`_event()`は未使用）。
    **無効化が要るのは、既に一度でも取得された(=キャッシュされ得た)既存event_idの内容を
    `flag_event.py`のnote/confirm/unconfirm/relate等で書き換えた時だけ**——自然失効を待たず
    `aws cloudfront create-invalidation --paths '/event?id=<書き換えたevent_id>'` を、
    実際に触ったevent_idぶんだけ列挙して打つこと。**`/event*`のようなワイルドカードは
    使わない**——触っていない確定済みイベント(実質1年キャッシュ)まで道連れで飛ばし、
    このキャッシュを入れた目的(閲覧人数比例のS3 GET対策)を自分で壊すことになる。
    `promote_event.py`が新規に発行するevent_idは書き込み前は誰にも取得され得ないため、
    初回フェッチは無効化なしで最新の内容を返す（新規作成の直後に反射的に打つ必要はない。
    詳細・実例は[tools/README.md](tools/README.md)参照）。
